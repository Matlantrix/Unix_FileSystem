...
void FileManager::deleteFile(const char* file_name)
{
	...
	string name = path.back();
	path.pop_back();
	int parent_inode_id = getInode(path);
	removeEntry(parent_inode_id, name.c_str());
}
...
void FileManager::createDir(const char* dir_name)
{
	...
	if (getInode(path) != -1) {
		cout << "cannot createDir '" << dir_name << "': Directory already exists";
		return;
	}
	if (path.size() == 0) return;
	string name = path.back();
	path.pop_back();
	int parent_node_id = getInode(path);
	if (parent_node_id == -1) {
	...
	disk.setInodeBitmap(inode_id, 1);
	disk.setDBlockBitmap(block_id, 1);
	addEntry(parent_node_id, name.c_str(), inode_id);
	addEntry(inode_id, "..", parent_node_id);
}
...
void FileManager::deleteDir(const char* dir_name)
{
	...
	unordered_map<string, int> inode_map;
	getInodeMap(dir_inode_id, inode_map);
	int parent_inode_id = inode_map[".."];
	inode_map.erase("..");
	for (auto iter = inode_map.begin(); iter != inode_map.end(); iter++) { //delete files in directory
	...
	removeEntry(parent_inode_id, path.back().c_str());
}
...
stack<string> FileManager::getCurrentPath()
{
	...
	while (inode_id != 0) {
		...
		parent_inode_id = inode_map[".."];
		getInodeMap(parent_inode_id, inode_map);
...
{
	vector<string> name_list, type_list, size_list, time_list;
	unordered_map<string, int> inode_map;
	...
	for (auto iter = inode_map.begin(); iter != inode_map.end(); ++iter) {
		...
		time_list.push_back(inode.getTimeCreated());
	...
	for (int i = 0; i < name_list.size(); ++i)
		cout << left << setw(NAME_LENGTH) << name_list[i] << setw(12) << type_list[i] << setw(12) << size_list[i] << time_list[i] << endl;
}
...
void FileManager::sum()
{
	...
	cout << left << "Used blocks: " << setw(12) << used_cnt << "Unused blocks: " << setw(12) << unused_cnt << endl;
}
